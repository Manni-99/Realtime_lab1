...
public class WashingProgram2 extends ActorThread<WashingMessage> {
    ...
    public void run() {
        try {
            ...
            if(io.getWaterLevel() <= 0)
            throw new InterruptedException("There is no water!");
            temp.send(new WashingMessage(this, WashingMessage.Order.TEMP_SET_60));
            WashingMessage ack12 = receive();
            System.out.println("gotTemp " + ack12);
            ...
            water.send(new WashingMessage(this, WashingMessage.Order.WATER_DRAIN));
            WashingMessage ack100 = receive();
            System.out.println("washing program 1 got " + ack100);
